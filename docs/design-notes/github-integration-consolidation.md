...
| **Credential-rotation blast radius** | Rotating the private key breaks BOTH agent operations AND login (broker uses client_secret, not the key, but conceptually one admin action in GitHub Settings touches both halves) | Independent rotation: agent key rotation doesn't affect login; client_secret rotation doesn't affect agents |
