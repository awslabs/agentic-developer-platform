...
Issue #1793 (Story D of E10 #1736): Repo picker API.
...
(issue #465 / #1672). This module lives in agent-context so the gateway
doesn't need to duplicate the logic — it's mounted via the conditional
AGENT_CONTEXT_ENABLED include path.
"""
...
logger = logging.getLogger("agent_context.api.github_app_service")
...
async def resolve_tenant_app_credentials(
    ...
    *,
    ...
    The installation_id is resolved from the ChannelTenantMap metadata
    (passed by caller) — not stored in the SM secret. So we return it
    ...
    try:
        ...
        raise ValueError(
            f"GitHub App not configured for tenant '{org_id}'. "
            "Ensure a GitHub App is installed via Settings > Connections."
        ) from exc
    ...
    if not app_id or not private_key:
        raise ValueError(
            f"GitHub App credentials incomplete for tenant '{org_id}' "
            f"(secret: {secret_id}). Missing app_id or private_key."
        )
...
async def list_accessible_repos(
    ...
    *,
    ...
    token = await mint_installation_token(
        app_id, private_key_pem, installation_id, http_client=http_client
    )
    ...
    try:
        ...
        while True:
            ...
            if (
                not repos
                or (total_count is not None and len(all_repos) >= total_count)
                or len(repos) < 100
            ):
                break
            gh_page += 1
