...
# Covers: chat_context, chat_artifacts, agent_memory, beads_manifest, sessions.
# =============================================================================

resource "aws_kms_key" "dynamodb" {
  description             = "Customer-managed KMS key for Agent Factory DynamoDB tables"
...
resource "aws_kms_alias" "dynamodb" {
  name          = "alias/adp-${var.environment}-agent-factory-dynamodb"
...
resource "aws_kms_key" "cloudwatch" {
  description             = "Customer-managed KMS key for Agent Factory CloudWatch Log Groups"
  deletion_window_in_days = 30
  enable_key_rotation     = true
...
resource "aws_kms_alias" "cloudwatch" {
  name          = "alias/adp-${var.environment}-agent-factory-cloudwatch"
...
resource "aws_kms_key" "secrets" {
  description             = "Customer-managed KMS key for Agent Factory Secrets Manager secrets"
  deletion_window_in_days = 30
  enable_key_rotation     = true
...
resource "aws_kms_alias" "secrets" {
  name          = "alias/adp-${var.environment}-agent-factory-secrets"
